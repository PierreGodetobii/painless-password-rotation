...
-p PH_SEPERATOR = Seperator for the words (string)
USAGE
)
...
function generate_password() {
  ...
  if [ -z {$VAULT_NAMESPACE+x} ]
  then
    NEWPASS=$(curl -sS --fail -X POST -H "X-Vault-Token: $VAULT_TOKEN" -H "Content-Type: application/json" --data ${DATA}   ${VAULT_ADDR}/v1/gen/password 2> /dev/null | jq -r '.data|.value')
  else
    NEWPASS=$(curl -sS --fail -X POST -H "X-Vault-Token: $VAULT_TOKEN" -H "X-Vault-Namespace: $VAULT_NAMESPACE" -H "Content-Type: application/json" --data ${DATA}   ${VAULT_ADDR}/v1/gen/password 2> /dev/null | jq -r '.data|.value')
  fi
  if [ $? -ne 0 ]; then
    return 1
  else
...
function genereate_passphrase() {
  DATA=$(jq --null-input \
    --arg words "${PH_WORDS}" \
    --arg seperator "${PH_SEPERATOR}" \
    '{"words": $words, "seperator": $seperator}')
  if [ -z {$VAULT_NAMESPACE+x} ]
  then
    NEWPASS=$(curl -sS --fail -X POST -H "X-Vault-Token: $VAULT_TOKEN" -H "Content-Type: application/json" --data ${DATA}   ${VAULT_ADDR}/v1/gen/passphrase 2> /dev/null | jq -r '.data|.value')
  else
    NEWPASS=$(curl -sS --fail -X POST -H "X-Vault-Token: $VAULT_TOKEN" -H "X-Vault-Namespace: $VAULT_NAMESPACE" -H "Content-Type: application/json" --data ${DATA}   ${VAULT_ADDR}/v1/gen/passphrase 2> /dev/null | jq -r '.data|.value')
  fi
  if [ $? -ne 0 ]; then
    return 1
  else
...
then
  ...
  generate_password \
  || { output_error "Failed to create new random password via vault" ; exit 3 ; } \
  && { printf "%b" "New random password generated" ; }
else
  # Generate new passphrase via vault
  genereate_passphrase \
    || { output_error "Failed to create new random passphrase via vault" ; exit 3 ; } \
    && { printf "%b" "New random passphrase generated" ; }
fi

# Store password inside vault
store_credentials_vault \
  || { output_error "Failed to store the credentials for ${USERNAME} inside vault. Not updating the local password" ; exit 3 ; } \
  && { printf "%b" "New credentials for ${USERNAME} stored" ; }

# Update local password
update_password \
  || { output_error "Failed to update the password for ${USERNAME}" ; exit 4 ; } \
  && { printf "%b" "Password for ${USERNAME} changed" ; NEWPASS="" ; exit 0 ; }
